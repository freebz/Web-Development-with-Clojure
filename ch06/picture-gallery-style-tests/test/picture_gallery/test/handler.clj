...
(deftest test-login
  ...
  (testing "password mismatch"
    (with-redefs [picture-gallery.models.db/get-user mock-get-user]
      (is
       (-> (request :post "/login" {:id "foo" :pass "123456" :__anti-forgery-token *anti-forgery-token*})
           app :headers (get "Set-Cookie") empty?))))
  
  (testing "user not found"
    (with-redefs [picture-gallery.models.db/get-user mock-get-user]
      (is
       (-> (request :post "/login" {:id "bar" :pass "12345" :__anti-forgery-token *anti-forgery-token*})
           app :headers (get "Set-Cookie") empty?)))))
